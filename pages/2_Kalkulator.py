...
def wczytaj_i_przygotuj_dane():
    try:
        ...
        # Normalizacja nazw kluczowych kolumn (Twoja mapa)
        ...
        # Standaryzacja zawartości (Przeniesiona tutaj, by oszczędzać RAM)
        ...
        # Budowa map pomocniczych (wewnątrz cache!)
...
# --- 3. TWOJE SERCE APLIKACJI (Logika inbredu - bez zmian) ---
...
# Pobieramy wszystko jednym wywołaniem
        ...
        if not wybrane_nazwy:
            ...
            with st.spinner("Analiza drzew genealogicznych..."):
                ...
                if finalne:
                    ...
                    st.error("Brak bezpiecznych dopasowań.")
                
                if raport_detektywa:
                    with st.expander("🕵️ ANALIZA KONFLIKTÓW"):
                        st.table(pd.DataFrame(raport_detektywa))
else:
    st.error("Nie udało się załadować plików. Sprawdź rodowody.parquet i Oferta CRV.xlsx.")